...
geometry_msgs::PoseStamped[] createLocalPath(geometry_msgs::PoseStamped pose_stamped){
    geometry_msgs::PoseStamped[] pathPoses;
    double phi_change, phi_start;
    geometry_msgs::Quaternion quat;
    phi_change = M_PI/2;
    
    quat = pose_stamped.pose.orientation;
    phi_start = convertPlanarQuat2Phi(quat);
    
    quat = convertPlanarPhi2Quaternion(phi_start + phi_change);
    pose_stamped.pose.orientation = quat;
    localPath.poses.push_back(pose_stamped);
    quat = convertPlanarPhi2Quaternion(phi_start - phi_change);
    pathPoses.push_back(pose_stamped);
    return pathPoses;
}
