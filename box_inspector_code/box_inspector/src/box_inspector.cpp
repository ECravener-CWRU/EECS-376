...
// orphan_models_wrt_world: vector of models that are seen in the box, but DO NOT belong in the box
  ...
  osrf_gear::Shipment cur_shipment;
  
  //Obtain models wrt box
  get_box_pose_wrt_world(box_pose_wrt_world);
  model_poses_wrt_box(cur_shipment);
  compute_shipment_poses_wrt_world(cur_shipment, box_pose_wrt_world,image_models_wrt_world);
  //affine_box_wrt_world = xformUtils_.transformPoseToEigenAffine3d(box_pose_wrt_world.pose);
  //for(int i=0;i<num_parts_seen;i++){
    
  //}
  for(int i=0;i<num_parts_desired+num_parts_seen;i++){
    d_catagorized[i] = false;
    i_catagorized[i] = false;
  }
  ROS_INFO("num_parts_seen: %d",num_parts_seen);
  ROS_INFO("num_parts_desired: %d",num_parts_desired);
  for(int i=0;i<num_parts_seen-1;i++) {
        ROS_INFO_STREAM("image" << image_models_wrt_world[i] << endl);
        ROS_INFO("%d",i);
