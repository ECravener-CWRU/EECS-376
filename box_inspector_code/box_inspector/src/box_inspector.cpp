...
// orphan_models_wrt_world: vector of models that are seen in the box, but DO NOT belong in the box
  ...
  bool i_catagorized[num_parts_seen];
  bool d_catagorized[num_parts_desired];
  bool s_type[num_parts_desired];  
  ...
  Eigen::Affine3d affine_model_wrt_world, affine_model_wrt_box, affine_box_wrt_world;
  geometry_msgs::Pose box_p_wrt_world, model_pose_wrt_world, difference_pose;
  geometry_msgs::PoseStamped box_pose_wrt_world;
  vector<osrf_gear::Model> image_models_wrt_world;
  
  int pos_tol = .03; //3cm position tolerance
  int ori_tol = .1; //.1rad orientation tolerance
  
  //Obtain models in box wrt world
  get_box_pose_wrt_world(box_pose_wrt_world);
  affine_box_wrt_world = xformUtils_.transformPoseToEigenAffine3d(box_pose_wrt_world.pose);
  for(int i=0;i<num_parts_seen;i++){
    model = box_inspector_image_.models[i]; 
    affine_model_wrt_box = xformUtils_.transformPoseToEigenAffine3d(model.pose);
    affine_model_wrt_world = affine_box_wrt_world * affine_model_wrt_box;
    model_pose_wrt_world = xformUtils_.transformEigenAffine3dToPose(affine_model_wrt_world);
    model.pose = model_pose_wrt_world;
    image_models_wrt_world.push_back(model);
    i_catagorized[i] = false;
  }
  string model_name;
  for(int i=0;i<num_parts_desired;i++){
    d_catagorized[i] = false;
    s_type[i] = false;
  }
  
  for(int i=0;i<num_parts_seen;i++) {
      for(int j=0;j<num_parts_desired;j++){     
	 if(desired_models_wrt_world[j].type.compare(image_models_wrt_world[i].type)==0){
            //if type is same, check proximity
            difference_pose = pose_difference(desired_models_wrt_world[j].pose, image_models_wrt_world[i].pose);
            if(within_spec(difference_pose)){
               i_catagorized[i] = true;
               d_catagorized[j] = true;
               satisfied_models_wrt_world.push_back(image_models_wrt_world[i]);
            }
         }
      }
  }
 for(int i=0;i<num_parts_seen;i++) {
      for(int j=0;j<num_parts_desired;j++){
        if(!i_catagorized[i] && !d_catagorized[j]){ //if not already catagorized check type
           if(desired_models_wrt_world[j].type.compare(image_models_wrt_world[i].type)==0){
              i_catagorized[i] = true;
              d_catagorized[j] = true;
              misplaced_models_actual_coords_wrt_world.push_back(image_models_wrt_world[i]);
 ...
 for(int i=0;i<num_parts_seen;i++) {
    if(!i_catagorized[i]){
      orphan_models_wrt_world.push_back(image_models_wrt_world[i]);
    }
 }
string box_name("shipping_box");
 for(int i=0;i<num_parts_desired;i++) {
    if(!d_catagorized[i]){
        ...
        if (model_name != box_name) {  //If not a shipping box add to orphan_models
          orphan_models_wrt_world.push_back(desired_models_wrt_world[i]);
        }
     }
 }
