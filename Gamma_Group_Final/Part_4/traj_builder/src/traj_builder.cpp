...
void curVelCallback(const geometry_msgs::Twist &cur_vel) {
    g_cur_vel = cur_vel;    
}
...
void TrajBuilder::build_braking_traj(geometry_msgs::PoseStamped start_pose,
        std::vector<nav_msgs::Odometry> &vec_of_states) {
    //FINISH ME!
    ROS_INFO("Testing braking...");
    //Subscriber to get cur speed
    ros::NodeHandle n; // two lines to create a publisher object that can talk to ROS
    ros::Subscriber cur_vel_subscriber = n.subscribe("cmd_vel",1,curVelCallback);
    
    nav_msgs::Odometry des_state;
    geometry_msgs::Pose cur_pos;
    cur_pos = start_pose.pose;
    double x_start = cur_pos.position.x;
    double y_start = cur_pos.position.y;
    double psi_start = convertPlanarQuat2Psi(cur_pos.orientation);
    des_state.header = start_pose.header; //really, want to copy the frame_id
    des_state.pose.pose = start_pose.pose; //start from here
    des_state.twist.twist = g_cur_vel; // start at cur_vel

    double t_ramp_lin = g_cur_vel.linear.x / accel_max_;
    double t_ramp_ang = g_cur_vel.angular.z / alpha_max_;
    int npts_ramp;
    int npts_ramp_lin = round(t_ramp_lin / dt_);
    int npts_ramp_ang = round(t_ramp_lin / dt_);
    if(npts_ramp_ang>npts_ramp_lin){
      npts_ramp = npts_ramp_ang;
    } else{
      npts_ramp = npts_ramp_lin;
    }
    double d_vel_lin = accel_max_*dt_; // incremental velocity changes for ramp-down
    double d_vel_ang = alpha_max_*dt_; // incremental velocity changes for ramp-down
    
    //Set start pos and vel
    double x_des = x_start; //start from here
    double y_des = y_start;
    double psi_des = psi_start;
    double speed_des = g_cur_vel.linear.x;
    double omega_des = g_cur_vel.angular.z;
    des_state.twist.twist.angular.z = omega_des;
    des_state.pose.pose.position = cur_pos.position;
    des_state.pose.pose.orientation = convertPlanarPsi2Quaternion(psi_des);
    ...
    for (int i = 0; i < npts_ramp; i++) {
        if(i<npts_ramp_lin){
          speed_des -= d_vel_lin; //Euler one-step integration
        } else{
          speed_des = 0;
        }
        if(i<npts_ramp_ang){
          omega_des -= d_vel_ang; //Euler one-step integration
        } else{
          omega_des = 0;
        }
        des_state.twist.twist.linear.x = speed_des;
    ...
    vec_of_states.push_back(des_state);
    ROS_INFO("Braking traj complete");
}
